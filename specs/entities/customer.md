...
| last_touch | timestamptz | nullable | Set to now() when Customer clicks «Показать QR-код» (scenario 03); null if action never taken. Scenario 17 does NOT update it — link button, no event reaches bot |
...
- last_touch: updated on every QR view; not cleared; null only if Customer has never viewed QR
